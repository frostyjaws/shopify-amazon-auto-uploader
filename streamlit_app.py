...
PRODUCT_TYPE = "LEOTARD"  # keep what works
...
# What shoppers see on the Color selector:
DISPLAY_COLOR = {
    "White": "White",
    "Pink": "Pink",
...
# Color-specific MAIN images (your links)
COLOR_MAIN_IMAGE = {
...
PRICE_TABLE = {
    ...
    # Short Sleeve, Beige (Natural) / Pink / Light Blue ($33.99) for selected sizes
...
def generate_amazon_json_feed(title, image_url):
    ...
    def sku_for(slug, size_text, color):
    ...
    for (size_text, color) in PRICE_TABLE.keys():
        ...
        child_sku = sku_for(slug, size_text, color)
        sleeve_value = "Short Sleeve" if "Short" in size_text else "Long Sleeve"

        # Use color-specific hero if available, else the uploaded one
        child_main_image = COLOR_MAIN_IMAGE.get(color_label, image_url)

        attributes = {
            "item_name": [{"value": f"{title} - {size_text} - {color_label}"}],
            "brand": [{"value": "NOFO VIBES"}],
            "item_type_keyword": [{"value": "infant-and-toddler-bodysuits"}],
            "product_description": [{"value": DESCRIPTION}],
            "bullet_point": [{"value": b} for b in BULLETS],
            ...
            # If Amazon enables explicit swatch slot in your node, we can add it here later.
        }
...
st.title("Shopify + Amazon Feed Uploader (Multi-Image, Size/Color Variations)")
...
if uploaded_files:
    ...
    for uploaded_file in uploaded_files:
        ...
        try:
            ...
            feed_fragment = json.loads(generate_amazon_json_feed(file_stem, image_url))
